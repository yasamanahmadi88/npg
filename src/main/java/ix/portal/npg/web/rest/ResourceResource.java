...
import ix.portal.npg.service.ResourceQueryService;
...
/**
 * REST controller for managing {@link ix.portal.npg.domain.ResourceEntity}.
 */
@RestController
@RequestMapping("/api")
public class ResourceResource extends BaseController {
    ...
    @Secured(ENTITY_NAME)
    public ResponseEntity<List<ResourceDTO>> getAllResources(ResourceCriteria criteria, Pageable pageable) throws IxssException {
        try {
    ...
    @Secured(ENTITY_NAME)
    public ResponseEntity<Long> countResources(ResourceCriteria criteria) throws IxssException {
        try {
    ...
    @Secured(ENTITY_NAME)
    public ResponseEntity<ResourceDTO> getResource(@PathVariable("id") Long id) throws IxssException {
        try {
    ...
    @Secured(ENTITY_NAME)
    public ResponseEntity<Void> deleteResource(@PathVariable("id") Long id) throws IxssException {
        try {
    ...
    @Secured(ENTITY_NAME)
    public ResponseEntity<List<ResourceDTO>> searchResources(String searchText, Pageable pageable) throws IxssException {
        log.debug("REST request to get Resources by searchText: {}", searchText);
        try {
            Page<ResourceDTO> page = resourceQueryService.searchByText(searchText, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
        } catch (Exception e) {
            handleException(e);
    ...
    @Secured(ENTITY_NAME)
    public ResponseEntity<List<ResourceDTO>> getAll(ResourceCriteria criteria) throws IxssException {
        try {
            log.debug("REST request to get Resources by criteria: {}", criteria);
            Page<ResourceDTO> page = resourceQueryService.findByCriteria(criteria, Pageable.unpaged());
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
        } catch (Exception e) {
            handleException(e);
