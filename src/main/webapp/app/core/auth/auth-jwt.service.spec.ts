...
describe('Auth JWT', () => {
  ...
  describe('Login', () => {
    it('should clear session storage and save in local storage when rememberMe is true', () => {
      ...
      // WHEN
      service.login({ username: 'John', password: '123', rememberMe: true, captchaId: 'captcha-id', captchaToken: 'captcha-token' }).subscribe();
    ...
    it('should clear local storage and save in session storage when rememberMe is false', () => {
      ...
      // WHEN
      service.login({ username: 'John', password: '123', rememberMe: false, captchaId: 'captcha-id', captchaToken: 'captcha-token' }).subscribe();
      httpMock.expectOne('api/authenticate').flush({ id_token: '1' });
