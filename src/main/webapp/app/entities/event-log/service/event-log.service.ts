...
export class EventLogService {
  ...
  protected convertDateFromClient(eventLog: IEventLog): IEventLog {
    return Object.assign({}, eventLog, {
      insertTimestamp: eventLog.insertTimestamp?.isValid() ? eventLog.insertTimestamp.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((eventLog: IEventLog) => {
        eventLog.insertTimestamp = eventLog.insertTimestamp ? dayjs(eventLog.insertTimestamp) : undefined;
      });
    }
    return res;
  }
