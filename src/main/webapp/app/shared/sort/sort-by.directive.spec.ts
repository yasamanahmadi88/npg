...
@NgModule({
  ...
  declarations: [SortDirective, SortByDirective],
  exports: [FontAwesomeModule, SortDirective, SortByDirective],
})
class SortByDirectiveTestingModule {}
...
describe('Directive: SortByDirective', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.overrideComponent(TestSortByDirectiveComponent, {
      add: { imports: [SortByDirectiveTestingModule] },
    });
  ...
  it('should initialize predicate, order, icon when initial component predicate is _score', () => {
    ...
    expect(sortByDirective.icon.iconName).toEqual('sort');
  ...
  it('should initialize predicate, order, icon when initial component predicate is same as column predicate', () => {
    ...
    expect(sortByDirective.icon.iconName).toEqual(faSortUp.iconName);
    expect(component.transition).toHaveBeenCalledTimes(0);
  });
  ...
  it('should update component predicate, order, icon when user clicks on column header', () => {
    ...
    expect(sortByDirective.icon.iconName).toEqual(faSortDown.iconName);
