...
import ix.portal.npg.service.mapper.UserMapper;
 ...
 */
@AutoConfigureMockMvc(addFilters = false)@WithMockUser(authorities = AuthoritiesConstants.ADMIN)
@IntegrationTest
class UserResourceIT {
    ...
    private void deleteExistingUserTestRows() {
        String usersToDelete = """
            select id from jhi_user
            where id in (1, 2)
               or lower(login) in ('admin', 'user', 'johndoe', 'jhipster')
               or lower(email) in ('admin@localhost', 'user@localhost', 'john.doe@jhipster.com', 'john.doe@localhost', 'jhipster@localhost', 'johndoe@localhost')
            """;

        jdbcTemplate.update("delete from jhi_user_authority where user_id in (" + usersToDelete + ")");
        jdbcTemplate.update("delete from jhi_user where id in (" + usersToDelete + ")");
