...
import ix.portal.npg.config.Constants;
...
import ix.portal.npg.service.UserService;
import ix.portal.npg.service.dto.AdminUserDTO;
import ix.portal.npg.service.dto.PasswordChangeDTO;
import ix.portal.npg.service.dto.UserDTO;
import ix.portal.npg.web.rest.vm.KeyAndPasswordVM;
import ix.portal.npg.web.rest.vm.ManagedUserVM;
...
class AccountResourceIT {
    ...
    void setupAccountTestData() {
        jdbcTemplate.update(
            ...
            where user_id in (
                ...
                where id >= 1000
                   or lower(login) = 'test'
                   or lower(login) like 'test-register-%'
                   or lower(login) like 'save-%'
                   or lower(login) like 'change-password%'
                   or lower(login) like 'password-reset%'
                   or lower(login) like 'finish-password-reset%'
                   or lower(login) in ('badguy', 'activate-account')
                   or lower(email) like 'test-register-%'
                   or lower(email) like 'save-%'
                   or lower(email) like 'change-password%'
                   or lower(email) like 'password-reset%'
                   or lower(email) like 'finish-password-reset%'
            )
            """
        );
    ...
    void testRegisterDuplicateLogin() throws Exception {
        ...
        Optional<User> testUser = userRepository.findOneWithAuthoritiesByEmailIgnoreCase("alice2@example.com");
        assertThat(testUser).isPresent();
        testUser.get().setActivated(true);
        userService.updateUser(new AdminUserDTO(testUser.get()));
    ...
    void testChangePassword() throws Exception {
        ...
        restAccountMockMvc
            .perform(
                post("/api/account/change-password")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(new PasswordChangeDTO(currentPassword, "NewPassword1!")))
            )
            .andExpect(status().isOk());
